...
object Fraction{
    ...
    def apply(num: Int, denom: Int, loggable: Boolean = false): Fraction =
        ...
        else new DeafaultImpl(num, denom)

    private class DeafaultImpl(val num: Int, val denom: Int) extends Fraction {
    ...
    private class LoggableImpl(num: Int, denom: Int) extends DeafaultImpl(num, denom) with Loggable{
        ...
        override def -(other: Fraction): Fraction = {
            log(timeStamp, " subtracting " + this.toString + " with " + other.toString)
            Fraction(this.num * other.denom - other.num * this.denom, this.denom * other.denom)
        }
